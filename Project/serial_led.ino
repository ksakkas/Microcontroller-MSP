...
Το σύστημα θα ενημερώνει τακτικά το χρήστη για την κατάσταση του συστήματος
*/
...
void loop() {              //Κύρια επαναληπτική συνάρτηση
  if(Serial.available())  //Αν η σειριακή είσοδος είναι διαθέσιμη
  {
    ...
    if(x==1) //Αν το x έχει την τιμή του ίση με 1
    {
      ...
      Serial.println("Το κόκκινο Led αναβοσβήνει κάθε 1 sec"); //Ενημερώνει για την κατάσταση του συστήματος 
      if(Serial.available())     //Αν η σειριακή είσοδος είναι διαθέσιμη
      {  
        ...
        {          
          Serial.println("Το κόκκινο Led είναι σβηστό"); //Ενημερώνει για την κατάσταση του συστήματος 
    ...
    else if (x==3)//Αν το x έχει την τιμή του ίση με 3
    {
      ...
      Serial.println("Το πράσινο Led αναβοσβήνει κάθε 0.5 sec");//Ενημερώνει για την κατάσταση του συστήματος
      if(Serial.available()) //Αν η σειριακή είσοδος είναι διαθέσιμη
      {  
        x = Serial.parseInt(); //Στην μεταβλητή x εκχωρούμε την τιμή που έδωσε ο χρήστης στην σειριακή μετατρέποντας το σε ακέραιο
        if(x==0) //Αν το x έχει την τιμή του ίση με 0
        ...
        { 
          Serial.println("Το πράσινο Led είναι σβηστό");//Ενημερώνει για την κατάσταση του συστήματος 
          break;   //Σταματάει την επανάληψη
    ...
    else if (x==6)//Αν το x έχει την τιμή του ίση με 6
    {
      digitalWrite(RED_LED, LOW);    //Δίνει λογική τιμή 0 (LOW) στο led δηλαδή το σβήνει
      digitalWrite(GREEN_LED, LOW);//Δίνει λογική τιμή 0 (LOW) στο led δηλαδή το σβήνει
      Serial.println("Το κόκκινο Led είναι σβηστό");//Ενημέρωση σχετικά με την συνολική λειτουργία του green led
      Serial.println("Το πράσινο Led είναι σβηστό");   //Ενημέρωση σχετικά με την συνολική λειτουργία του green led
    }
