...
3-4 Green Led
5-6 Both
(Odd numbers refer to the LEDs and odd numbers to the LEDs off)

The red LED will flash with a frequency of 1Hz while the green with a frequency of 2Hz.

Use a different meter for each LED

The system will regularly inform the user about the status of the system
*/
...
void loop() {              //Main iterative function
  if(Serial.available())  //If serial input is available
  {
    ...
    if(x==1) //If x has its value equal to 1
    {
      ...
      Serial.println("The red LED flashes every 1 sec"); //Informs about the status of the system 
      if(Serial.available())     //If serial input is available
      {  
        ...
        {          
          Serial.println("The red LED is off"); //Informs about the status of the system
    ...
    {
      while(x=3) { //As long as x = 3 => create a repetitive loop infinite
      ...
      Serial.println("The green LED flashes every 0.5 sec");//Informs about the status of the system
      if(Serial.available()) //If serial input is available
      {  
        x = Serial.parseInt(); //In the variable x we ​​assign the value given by the user to the serial by converting it to integer
        if(x==0) //If x has its value equal to 0
        ...
        { 
          Serial.println("The green LED is off");//Informs about the status of the system
          break;   //Stops the repetition
    ...
    else if (x==5)//If x has its value equal to 5
    {
      ...
      r++;  //Increases the counter by one
      g++;  //Increases the counter by one 
      Serial.println("Το κόκκινο Led αναβοσβήνει κάθε 1 sec");//Informs about the status of the system
      Serial.println("Το πράσινο Led αναβοσβήνει κάθε 0.5 sec");//Informs about the status of the system
      if(Serial.available())//If serial input is available
      {  
        x = Serial.parseInt(); //In the variable x we ​​assign the value given by the user to the serial by converting it to integer
        if(x==0)//If x has its value equal to 0
    ...
    else if (x==6)//If x has its value equal to 6
    {
      digitalWrite(RED_LED, LOW);    // Gives a reasonable value of 0 (LOW) to the led, ie it turns it off
      digitalWrite(GREEN_LED, LOW);//Gives a reasonable value of 0 (LOW) to the led, ie it turns it off
      Serial.println("The red LED is off");//Information on the overall operation of the red led
      Serial.println("The green LED is off");   //Information on the overall operation of the green led
    }
